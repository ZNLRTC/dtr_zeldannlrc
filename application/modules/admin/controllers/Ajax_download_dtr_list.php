...
class Ajax_download_dtr_list extends CI_Controller {
                ...
                if($check_user):
                    ...
                    foreach ($period as $key => $value):
                        ...
                        foreach($h_list as $h_list):
                            if($h_list['date'] == $date):
                                ...
                                switch($h_list['type']){
                                    ...
                                    default:
                                        $holiday = "Regular Overtime";
                                    break;
                                }
                            endif;
                        endforeach;

                        $lr         = $this->request_leave_model->get_one_by_where(["user_id"=>$user['id'], "date"=>$date, "status" => "approved"]);
                        //To cater old data prior to system revisions
                        //$leave      = ($lr && ($lr['status'] !== "denied" || $lr['status'] !== 'cancelled')) ? "EB9C5C" : null;
                        $leave      = $lr ? "EB9C5C" : null;
                        $check_date = $this->dtr_model->get_one_by_where(["user_id"=>$user['id'], "date"=>$date]);
                        $check_ot_logs = $this->overtime_logs_model->get_one_by_where(['user_id' => $user['id'], 'date' => $date]);
                        ...
                        if($leave):
                            ...
                            //start Migs
                            elseif($check_ot_logs):
                                if($check_ot_logs['break_in'] != NULL && $check_ot_logs['break_out'] != NULL){
                                    $break_ot = date('h:i a', strtotime($check_ot_logs['break_in'])) .'-'. date('h:i a', strtotime($check_ot_logs['break_out']));
                                }else{
                                    $break_ot = 'No Break';
                                }

                                if($check_ot_logs['time_out'] != NULL){
                                    
                                    $ot_in = new DateTime($check_ot_logs['time_in']);
                                    $ot_out = new DateTime($check_ot_logs['time_out']);
                                    $ot_b_in = new DateTime($check_ot_logs['break_in']);
                                    $ot_b_out = new DateTime($check_ot_logs['break_out']);
                                    $tito_diff = $ot_in->diff($ot_out);
                                    $bibo_diff = $ot_b_in->diff($ot_b_out);
                                    $tito_minutes = ($tito_diff->h * 60) + $tito_diff->i;
                                    $bibo_minutes = ($bibo_diff->h * 60) + $bibo_diff->i;
                                    $max_bibo_minutes = max($bibo_minutes, 60);
                                    $total_minutes = $tito_minutes - $max_bibo_minutes;
                                    $total_ot_decimal = $total_minutes / 60;
                                    $total_ot = round($total_ot_decimal, 2);
                                    $ot_start_end = date('h:i a', strtotime($check_ot_logs['time_in'])) .'-'. date('h:i a', strtotime($check_ot_logs['time_out']));

                                }else{
                                    $total_ot = 'On going OT';
                                    $ot_start_end = date('h:i a', strtotime($check_ot_logs['time_in'])) .'- On-going';
                                }
                                ...
                                $spreadsheet->setActiveSheetIndex(0)
                                ->getStyle('A'.$row_num.':J'.$row_num)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
                                ->getStartColor()->setARGB('FFC1C3');
                                
                                $spreadsheet->setActiveSheetIndex(0)
                                    ->setCellValue("B".$row_num, 'No Schedule')
                                    ->setCellValue("C".$row_num, date( "h:i a",strtotime($check_ot_logs['time_in'])))
                                    ->mergeCells('D'.$row_num.':E'.$row_num)
                                    ->setCellValue("D".$row_num, $break_ot)
                                    ->setCellValue("F".$row_num, ($check_ot_logs['time_out']) ? date( "h:i a", strtotime($check_ot_logs['time_out'])) : "")
                                    ->setCellValue("I".$row_num, 'Office')
                                    ->setCellValue("J".$row_num, $total_ot);

                                $spreadsheet->setActiveSheetIndex(0)
                                    ->setCellValue("H".$row_num, $ot_start_end)
                                    ->getStyle('H'.$row_num)->getFont()->getColor()->setARGB("039487");
